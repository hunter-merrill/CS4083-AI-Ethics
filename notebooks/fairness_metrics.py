...
def equal_opportunity(data, majority, minority):
    conf_matrix_1 = confusion_matrix(data[(data["Ethnicity"] == majority)]["TrueDiagnosis"], data[(data["Ethnicity"] == majority)]["PredictedDiagnosis"]).ravel().tolist()
    conf_matrix_2 = confusion_matrix(data[(data["Ethnicity"] == minority)]["TrueDiagnosis"], data[(data["Ethnicity"] == minority)]["PredictedDiagnosis"]).ravel().tolist()
    
    return abs(conf_matrix_1[3]/(conf_matrix_1[3] + conf_matrix_1[1]) - conf_matrix_2[3]/(conf_matrix_2[3] + conf_matrix_2[1]))
